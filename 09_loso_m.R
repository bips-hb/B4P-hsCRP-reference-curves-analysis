...
## Program name:  09_loso_m.R
## Author:        Jiayi Zeng
## R-Version:     4.5.2
## version/ Date: V 1.0/ 2026-05-25
##
## Purpose:       leave one study out validation (males)
##
...
### first part of LOSO 
list_mods_1stpart <- LOSO_CV_mods_only(mod = mod_logRA_m,
                                       analysis_data = g1_data_CRP_m,
                                       ana_data_factor = g1_data_CRP_m$study_acronym)

save(list_mods_1stpart, file = "./30_Analysis/CRP/data/LOSO_crp_part1_m.RData")
# load("./30_Analysis/CRP/data/LOSO_crp_part1_m.RData")   
...
#2nd part of LOSO
list_mods_2ndpart <- LOSO_CV_mods_only(mod = mod_age_GB1_m_all2,
                                       analysis_data =g2_data_CRP_m,
                                       ana_data_factor = g2_data_CRP_m$study_acronym)


save(list_mods_2ndpart, file = "./30_Analysis/CRP/data/LOSO_crp_part2_m.RData")
# load("./30_Analysis/CRP/data/LOSO_crp_part2_m.RData")   
...
left1 <- vapply(list_mods_1stpart, find_leftout, "", data = g1_data_CRP_m)
left2 <- vapply(list_mods_2ndpart, find_leftout, "", data = g2_data_CRP_m)
...
svg("./30_Analysis/CRP/results/Fig_S12A_CRP_LOSO_m.svg")
...
ndm <- data.frame(
  age = age_seq,
  study_acronym = factor(levels(g1_data_CRP_m$study_acronym)[1],
                         levels = levels(g1_data_CRP_m$study_acronym))
)

t1m <- predict(mod_logRA_m, newdata = ndm, type = "terms")
P1m <- plogis(t1m[, term_name_part1] + attr(t1m, "constant"))
t2m_mu <- predict(mod_age_GB1_m_all2, newdata = ndm, type = "terms", what = "mu")

mum    <- plogis(t2m_mu[, term_name_part2] + attr(t2m_mu, "constant")) #without random effect
sigmam <- as.numeric(predict(mod_age_GB1_m_all2, newdata = ndm, type = "response", what = "sigma"))
num    <- as.numeric(predict(mod_age_GB1_m_all2, newdata = ndm, type = "response", what = "nu"))
taum   <- as.numeric(predict(mod_age_GB1_m_all2, newdata = ndm, type = "response", what = "tau"))

P2m_50 <- Calc_Perc_2(P1m, 0.50)
Qm50 <- gamlss.dist::qGB1(P2m_50, mum, sigmam, num, taum) + 0.02
P2m_95 <- Calc_Perc_2(P1m, 0.95)
Qm95 <- gamlss.dist::qGB1(P2m_95, mum, sigmam, num, taum) + 0.02
...
for (j in seq_along(idx_ok)) {
  ...
  # part 1(terms)
  data_LOSO <- droplevels(subset(g1_data_CRP_m, study_acronym != left_out))
  nd1 <- data.frame(
    age = age_seq,
  ...
  data_LOSO <- droplevels(subset(g2_data_CRP_m, study_acronym != left_out))
  nd2 <- data.frame(
    age = age_seq,
    study_acronym = factor(levels(data_LOSO$study_acronym)[1],
                           levels = levels(data_LOSO$study_acronym))
...
# black lines
idx_m <- ndm$age <= x_end
lines(ndm$age[idx_m], Qm95[idx_m] , col = "black", lwd = 3)
lines(ndm$age[idx_m], Qm50[idx_m] , col = "black", lwd = 3)

# label of percentile
text(label_x, tail(Qm95[idx_m] , 1), "P95", pos = 4, cex = 0.85, xpd = TRUE, offset = 0.6)
text(label_x, tail(Qm50[idx_m] , 1), "P50", pos = 4, cex = 0.85, xpd = TRUE, offset = 0.6)
