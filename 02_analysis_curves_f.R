...
## Program name:  02_analysis_curves_f.R
## Author:        Timm Intemann
## R-Version:     4.5.2
## version/ Date: V 1.0/ 2026-05-13
##
## Purpose:       Calculate reference curves for CRP (females)
##
...
studies <- unique(ana_data_CRP_f$study_acronym)
...
svg("./30_Analysis/CRP/results/Fig_S5B_fitted_log_reg_1st_Part_f.svg")
...
newdata <- data.frame(age = age_seq,
                        study_acronym = "")
mu_terms_1_part_global <- predict(mod_logRA_f, newdata = newdata, type = "terms")
...
for (i in seq_along(studies)) {
  ...
  pred_prob <- predict(mod_logRA_f, newdata = newdata_i, type = "response")
...
start_time <- Sys.time()
mod_GG_f <- gamlss(crp ~ 1,
                   data = g2_data_CRP_f, 
                   family = GG(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GG_f <- end_time - start_time

start_time <- Sys.time()
mod_GG_f_shift <- gamlss(shift_crp ~ 1,
                         data = g2_data_CRP_f, 
                         family = GG(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GG_f_shift <- end_time - start_time

start_time <- Sys.time()
mod_GB2_f <- gamlss(crp ~ 1,
                    data = g2_data_CRP_f, 
                    family = GB2(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GB2_f <- end_time - start_time

start_time <- Sys.time()
mod_GB2_f_shift <- gamlss(shift_crp ~ 1,
                          data = g2_data_CRP_f, 
                          family = GB2(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GB2_f_shift <- end_time - start_time

start_time <- Sys.time()
mod_GIG_f_shift <- gamlss(shift_crp ~ 1,
                          data = g2_data_CRP_f, 
                          family = GIG(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GIG_f_shift <- end_time - start_time

start_time <- Sys.time()
mod_GB1_f_shift <- gamlss(shift_crp ~ 1,
                          data = g2_data_CRP_f, 
                          family = GB1(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_GB1_f_shift <- end_time - start_time

#### 2.2.1.1 Diagnostics (no age) -------------
BIC_CRP_noage_f <- AIC(mod_GG_f,
                       mod_GG_f_shift,
                       mod_GB2_f,
                       mod_GB2_f_shift,
                       mod_GIG_f_shift,
...
write.csv(BIC_CRP_noage_f, "./30_Analysis/CRP/results/Tab_S8B_BIC_CRP_2nd_Part_noage_f.csv")

#### 2.2.1.2 Histogram and densities (no age) -------------

colors_dist <- colors[c(1, 9)]
svg("./30_Analysis/CRP/results/Fig_S6B_hist_dens_noage_f.svg")
#histogram
wtd.hist(g2_data_CRP_f$shift_crp, 
         weight = g2_data_CRP_f$W, 
         breaks = 100, 
...
abline(v = wtd.quantile(g2_data_CRP_f$shift_crp, 
                        weight = g2_data_CRP_f$W,
                        probs = 0.5), 
       col = "black",
       lty = 1,
...
distributions <- list(dGG, 
                      dGB1) 
models <- list(mod_GG_f_shift, mod_GB1_f_shift) 
...
start_time <- Sys.time()
mod_age_GB1_f_all <- gamlss(shift_crp ~ pb(age, max.df = 8, method = "GAIC", k=k2) + random(study_acronym),
                            ...
                            family = GB1(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_age_GB1_f_all <- end_time - start_time

start_time <- Sys.time()
mod_age_GB1_f_all2 <- gamlss(shift_crp ~ pb(age, max.df = 8, method = "GAIC", k=k2) + random(study_acronym),
                            ...
                            family = GB1(), weights = W, control = con)
end_time <- Sys.time()
time_mod_age_GB1_f_all2 <- end_time - start_time

start_time <- Sys.time()
mod_age_GG_f_pre_all <- gamlss(shift_crp ~ pb(age, max.df = 8, method = "GAIC", k=k2) + random(study_acronym),
                           sigma.fo = ~ pb(age, max.df = 4, method = "GAIC", k=k2),
                           nu.fo = ~ 1,
                           data = g2_data_CRP_f, 
                           family = GG(), weights = W, control = con) 
end_time <- Sys.time()
time_mod_age_GG_f_pre_all <- end_time - start_time

#### 2.2.2.1 Diagnostics ------------------------
svg("./30_Analysis/CRP/results/Fig_S7B1_WP_final_2nd_part_f.svg")
wgt_wp(mod_age_GG_f_pre_all$residuals,
       mod_age_GG_f_pre_all$weights, 
       mod_name_title = paste("(shifted GG with", round(mod_age_GG_f_pre_all$df.fit), "degrees of freedom)"))
dev.off()
svg("./30_Analysis/CRP/results/Fig_S7B2_WP_final_2nd_part_f.svg")
wgt_wp(mod_age_GB1_f_all$residuals,
       mod_age_GB1_f_all$weights, 
       mod_name_title = paste("(shifted GB1 with", round(mod_age_GB1_f_all$df.fit), "degrees of freedom)"))
dev.off()
svg("./30_Analysis/CRP/results/Fig_S7B3_WP_final_2nd_part_f.svg")
wgt_wp(mod_age_GB1_f_all2$residuals,
       mod_age_GB1_f_all2$weights, 
       mod_name_title = paste("(shifted GB1 with", round(mod_age_GB1_f_all2$df.fit), "degrees of freedom)"))
dev.off()

BIC_summary_CRP_best_f <- AIC(mod_age_GB1_f_all, 
                              mod_age_GB1_f_all2, 
                              mod_age_GG_f_pre_all, k = k2)

write.csv(BIC_summary_CRP_best_f, "./30_Analysis/CRP/results/Tab_S9B_BIC_part2_f.csv")
...
svg("./30_Analysis/CRP/results/Fig_S8B_2nd_mod-comp_f.svg")
...
add_gridlines(x = 0:30, y = 0:10/10,
              P = 23.5, cent = cent, y_P = c(0, 0.1, 0.65))
...
add_plot_global_curves_GG(mod = mod_age_GG_f_pre_all, newdata, cent, 
                          mu_term_name = 'pb(age, max.df = 8, method = "GAIC", k = k2)',
                          col_curve = "darkgreen")

add_plot_global_curves_GB1(mod = mod_age_GB1_f_all, newdata, cent, 
                       mu_term_name = 'pb(age, max.df = 8, method = "GAIC", k = k2)',
                       col_curve = "darkblue")

add_plot_global_curves_GB1(mod = mod_age_GB1_f_all2, newdata, cent, 
                       mu_term_name = 'pb(age, max.df = 8, method = "GAIC", k = k2)',
                       col_curve = "black")
legend("topleft", 
       inset = 0.02,
       legend = c(paste("Shifted GG with", round(mod_age_GG_f_pre_all$df.fit), "degrees of freedom"), 
                  paste("Shifted GB1 with", round(mod_age_GB1_f_all$df.fit), "degrees of freedom"), 
                  paste("Shifted GB1 with", round(mod_age_GB1_f_all2$df.fit), "degrees of freedom")
                  ), 
...
dev.off()# -> final model: mod_age_GB1_f_all2

## save final model 
save(mod_age_GB1_f_all2, file = "./30_Analysis/CRP/data/mod_age_GB1_f_all2.RData")
#load("./30_Analysis/CRP/data/mod_age_GB1_f_all2.RData")

#Plot final 2nd part of model including study specific curves
newdata <- data.frame(age = 10:225/10)
newdata$study_acronym <- " "

svg("./30_Analysis/CRP/results/Fig_S9B_2nd_Part_final_f.svg")
plot(c(-100, 10000), rep(20, times = 2), 
     ylim = c(0, 1), xlim = range(newdata$age) * c(1, 1.06), 
     col = grey(0.9),
     xlab = "Age (yrs)", ylab = "hs-CRP (shifted) (mg/L)",
...
add_plot_all_study_curves_GB1(mod = mod_age_GB1_f_all2, 
                              newdata = newdata, 
                              cent = cent,
...
add_plot_global_curves_GB1(mod = mod_age_GB1_f_all2, 
                           newdata = newdata, 
                           cent = cent, 
                           mu_term_name = 'pb(age, max.df = 8, method = "GAIC", k = k2)',
                           col_curve = "black")
...
All_perc_comb_f <- calc_all_perc_hurdle(age_seq = age_seq, 
                                        mod1 = mod_logRA_f, 
                                        mod2 = mod_age_GB1_f_all2,
                                        studies = studies)
#Select columns
Tab_S7B <- All_perc_comb_f$global[,c("age",
                                     "P_1",
                                     "Perc_comb_50",
                                     "Perc_comb_75",
                                     "Perc_comb_85",
...
#Unit transformation
Tab_S7B <-  Tab_S7B * rep(c(1, 100, rep(10, times = 7)), each = nrow(Tab_S7B))
#Rounding
Tab_S7B <- round(Tab_S7B, 2)
#Name columns
names(Tab_S7B) <- c("Age (yrs)",
                    "Prob. (hs-CRP <= 0.2 mg/L)",
                    "P50", "P75", "P85", "P90", "P95", "P97", "Cut-off")
#Write csv
write.csv(Tab_S7B, "./30_Analysis/CRP/results/Tab_S7B_All_perc_comb_f.csv", row.names = FALSE)

##3.2 Plotting  ----------------------

###3.2.1 Cutoffs ------------------

pdf("./30_Analysis/CRP/results/Fig_3B_CRP_curves_cutoff_f.pdf")
...
add_gridlines(x = 0:30, y = 0:10/10,
              P = 23, cent = round(All_perc_comb_f$global$Perc_comb_18[1]*100, 1), 
              y_P = All_perc_comb_f$global$Perc_comb_cutoff[All_perc_comb_f$global$age==22.5] - 0.04)

add_gridlines_simple(x = c(18), y = 0.3, col = "darkgrey", lwd = 3)

lines(All_perc_comb_f$global$age, 
      All_perc_comb_f$global$Perc_comb_cutoff, 
      lwd = 3, lty = 1, col = "black")

for (i in 1:(length(studies))){
  lines(All_perc_comb_f[[i]]$age, 
        All_perc_comb_f[[i]]$Perc_comb_cutoff, 
...
pdf("./30_Analysis/CRP/results/Fig_2B_curves_global_total_f.pdf")
par(mar = c(5, 4, 4, 5) + 0.1)
plot(c(-100, 10000), rep(20, times = 2), 
     ylim = c(0, 1), xlim = c(0.5, 23.5) , 
     col = grey(0.9),
...
add_gridlines(x = 0:30, y = 0:10/10,
              ...
              y_P = c(All_perc_comb_f$global$Perc_comb_50[All_perc_comb_f$global$age==22.5],
                      All_perc_comb_f$global$Perc_comb_75[All_perc_comb_f$global$age==22.5],
                      All_perc_comb_f$global$Perc_comb_85[All_perc_comb_f$global$age==22.5],
                      All_perc_comb_f$global$Perc_comb_90[All_perc_comb_f$global$age==22.5],
                      All_perc_comb_f$global$Perc_comb_95[All_perc_comb_f$global$age==22.5],
                      All_perc_comb_f$global$Perc_comb_97[All_perc_comb_f$global$age==22.5])
              )
...
lines(All_perc_comb_f$global$age, 
      All_perc_comb_f$global$Perc_comb_50, 
      lwd = 3, lty = 1, col = "black")
lines(All_perc_comb_f$global$age, 
      All_perc_comb_f$global$Perc_comb_75, 
      lwd = 3, lty = 1, col = "black")
lines(All_perc_comb_f$global$age, 
      All_perc_comb_f$global$Perc_comb_85, 
      lwd = 3, lty = 1, col = "black")
lines(All_perc_comb_f$global$age,
      All_perc_comb_f$global$Perc_comb_90,
      lwd = 3, lty = 1, col = "black")
lines(All_perc_comb_f$global$age, 
      All_perc_comb_f$global$Perc_comb_95, 
      lwd = 3, lty = 1, col = "black")
lines(All_perc_comb_f$global$age,
      All_perc_comb_f$global$Perc_comb_97, 
      lwd = 3, lty = 1, col = "black")
...
# all global curves
svg("./30_Analysis/CRP/results/Fig_S10B_CRP_curves_studyspec_total_f.svg")
par(mar = c(5, 4, 4, 5) + 0.1)
plot(c(-100, 10000), rep(20, times = 2), 
     ylim = c(0, 1), xlim = c(0.5, 23.5) , 
     col = grey(0.9),
...
text(23.5, All_perc_comb_f$global$Perc_comb_50[All_perc_comb_f$global$age==22.5], "P50")
text(23.5, All_perc_comb_f$global$Perc_comb_90[All_perc_comb_f$global$age==22.5], "P90")

for (i in seq_along(studies)) {
  nm <- studies[i]
  df <- All_perc_comb_f[[i]]
  sel <- df$age < 22.5
...
All_perc_comb_long_f <- calc_all_perc_hurdle(age_seq = 100:2250/100, 
                                             mod1 = mod_logRA_f, 
                                             mod2 = mod_age_GB1_f_all2,
                                             studies = studies)

write.csv(All_perc_comb_long_f, "./30_Analysis/CRP/data/All_perc_comb_long_f.csv", row.names = FALSE)

#4 Association of repeated CRP values in the reference system -----------------

## 4.1 Select repeated measurements data -------
rep_ana_data <- ana_data_CRP_f[ana_data_CRP_f$age<=22.5, 
...
rep_ana_data_under18$crp_high_under18 <- rep_ana_data_under18$P_crp>=0.899
