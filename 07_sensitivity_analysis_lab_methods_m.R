...
## Program name:  06_sensitivity_analysis_lab_methods_m.R
## Author:        Jiayi Zeng
## R-Version:     4.5.2
## version/ Date: V 1.0/ 2026-05-25
##
## Purpose:       Sensitivity analysis for different lab methods (males)
##
## Program specification: 
## - Prerequisites: none
## - Read:  *Data: "./data/ana_data_20260424.rds"
##          *Cutoff file: "./30_Analysis/CRP/data/All_perc_comb_long_m.csv"
##          *Model: "./30_Analysis/CRP/data/mod_age_GB1_m_all2.RData"
## ***************************************************************************
...
load("./30_Analysis/CRP/data/mod_age_GB1_m_all2.RData")   #model, object: mod_age_GB1_m_all2 

# select males
ana_data_CRP_m <- subset(ana_data_CRP, sex == 1)

# gamlss analysis data for two part models
g1_data_CRP_m <- ana_data_CRP_m[, c("age", "methods","crp_le02", "study_acronym", "W")]
...
g1_data_CRP_m$methods <- droplevels(factor(g1_data_CRP_m$methods))
g2_data_CRP_m$methods <- droplevels(factor(g2_data_CRP_m$methods))
...
ref_meth <- names(sort(table(g1_data_CRP_m$methods), decreasing = TRUE))[1]
g1_data_CRP_m$methods <- relevel(g1_data_CRP_m$methods, ref = ref_meth)
g2_data_CRP_m$methods <- relevel(g2_data_CRP_m$methods, ref = ref_meth)
...
mod_log_m <- gamlss(crp_le02 ~ pb(age, max.df = 8, method = "GAIC", k=k),
                    data = g1_data_CRP_m, 
...
mod_logRA_meth_m <- gamlss(
  crp_le02 ~ pb(age, max.df = 8, method = "GAIC", k=k) + methods + random(study_acronym),
  data = g1_data_CRP_m, family = BI(), weights = W, control = con
)
...
mod_age_GB1_m_all2_m <- gamlss(
  ...
  data = g2_data_CRP_m, family = GB1(), weights = W, control = con
)

#save the model (mod_age_GB1_m_all2_m)
save(mod_age_GB1_m_all2_m, file = "./30_Analysis/CRP/data/methods_added_mod_age_GB1_m_all2_meth.RData")
# load("./30_Analysis/CRP/data/methods_added_mod_age_GB1_m_all2_meth.RData")   

## ****************************************************************************
# 4 Combine 1st and 2nd Part ------------------------
## ****************************************************************************

#All_perc_comb_m.rds saved
df <- read.csv("./30_Analysis/CRP/data/All_perc_comb_long_m.csv")
df_global <- df %>%
  select(starts_with("global."))
names(df_global) <- sub("^global\\.", "", names(df_global))
All_perc_comb_m <- list()
All_perc_comb_m[["global"]] <- df_global
write_rds(All_perc_comb_m, "./30_Analysis/CRP/data/All_perc_comb_long_m.rds")

## 4.1 Methods added global data compiling ---------------

levels_methods <- levels(model.frame(mod_logRA_meth_m)$methods)

levels_study   <- levels(g1_data_CRP_m$study_acronym)

All_perc_comb_m_meth <- list()
...
for (m in levels_methods) {
  ...
  tt <- predict(mod_logRA_meth_m, newdata = newdata, type = "terms")
  ...
  tt_mu <- predict(mod_age_GB1_m_all2_m, newdata = newdata, type = "terms", what = "mu")
  ...
  sigma <- predict(mod_age_GB1_m_all2_m, newdata = newdata, type = "response", what = "sigma")
  nu    <- predict(mod_age_GB1_m_all2_m, newdata = newdata, type = "response", what = "nu")
  tau   <- predict(mod_age_GB1_m_all2_m, newdata = newdata, type = "response", what = "tau")
  ...
  All_perc_comb_m_meth[[m]] <- data.frame(
    age = age_seq,
    P_1 = P_1,
...
All_perc_comb_m_meth <- add_cutoffs(cutoff = 0.3, shift = 0.02, all_perc_list = All_perc_comb_m_meth)

write.csv(All_perc_comb_m_meth, "./30_Analysis/CRP/data/sensitivity_methods_added_All_perc_comb_m.csv")
write_rds(All_perc_comb_m_meth, "./30_Analysis/CRP/data/sensitivity_methods_added_All_perc_comb_m.rds")

## 4.2 Generate 3-methods and global curves-----------------

### 4.2.1 Data load----------
# All_perc_comb_m <- readRDS("./30_Analysis/CRP/data/All_perc_comb_long_m.rds") 
# All_perc_comb_m_meth <- readRDS("./30_Analysis/CRP/data/sensitivity_methods_added_All_perc_comb_m.rds")

orig_global <- All_perc_comb_m[["global"]]
methods_vec <- names(All_perc_comb_m_meth)
...
svg("./30_Analysis/CRP/results/Fig_S11A_methods_m.svg",
    width = 7, height = 6)
...
summ_m <- data.frame(methods = as.character(methods_vec)) %>%
  left_join(df_counts_m, by = "methods")

#prepare legends including summary statistics
legend_methods <- sprintf("%s (n = %s, waves = %d)",
                          summ_m$methods, format(summ_m$n, big.mark=","), summ_m$waves)
